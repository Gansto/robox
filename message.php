...
$value = "fyxf0724cs";
setcookie("cookie", $value, time()+120);
...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if($_POST["chec"] == $hash) {
		...
		$sql = "insert into message(id, name, sortnum, phone, email, content, ip, create_time, state) values($id, '$name', $sortnum, '$phone', $num, '$content', '$ip', '$create_time', 0)";
		$rst = $db->query($sql);
		$db->close();
		if ($rst){
		...
		$db->close();
		echo "<script>alert('请重新进行提交！');history.back(-1);</script>";
		exit;
	}
}
?>
