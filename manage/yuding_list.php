...
$listUrl = "yuding_list.php?page=$page";
$viewUrl = "yuding_view.php?page=$page";

//连接数据库
$db = new onlyDB($config["db_host"], $config["db_user"], $config["db_pass"], $config["db_name"]);

if($id !=""){
	$sql = "delete from odermessage where id =$id";
	if (!$db->query($sql))
	{
		$db->close();
		info("删除资料失败！");
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$sql = "delete from odermessage where id in (" . implode(",", $id_array) . ")";
	$rst = $db->query($sql);
                            ...
                            <form name="listForm" method="post">
                                ...
                                <table class="listTable table table-striped table-bordered table-hover table-checkable order-column dataTable no-footer" id="sample_1">
                                    ...
                                    <tbody>
					    				...
					    				//总记录数
					                    $sql = "select count(*) as cnt from odermessage";
					                    $rst = $db->query($sql);
						                ...
						                $sql = "select id, sortnum, name, phone, email, content, create_time, state from odermessage order by sortnum desc";
